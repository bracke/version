- Add: git's names for the four commands this CLI spells differently -- `add`, `commit`, `rm` and `fsck` now dispatch to `stage`, `save`, `remove` and `verify`, through `version <name>`, `version <name> --help` and `version help <name>`. These alias the command name only: the commands keep this CLI's flags and output, so `version add` is not `git add` (it prints `staged X`, and rejects `-A`/`-u`/`-n`). Documented as such rather than as parity.
- Fix: `rev-list` and `log` share one revision-argument parser and one history walk, as git does in `setup_revisions()`. `rev-list` had its own depth-first walk that ignored committer-date ordering and rejected almost every flag as an unknown argument, while `Version.History.Rev_List` -- which already implemented git's date-ordered two-frontier walk -- went unused; `log` re-walked history a third way and could not take a range, an exclusion or a path at all. Both now accept several revisions, `A..B` and `A...B` ranges, `^exclusions`, `--` path limits with git's default history simplification, `--skip`, `--reverse`, `--merges`/`--no-merges`, `--min-parents`/`--max-parents`, `--first-parent` and `--topo-order` (git's LIFO topological sort, which keeps a line of development contiguous); `rev-list` additionally gained `--parents`, `--oneline`, `--objects`, `--branches` and `--tags`. An operand that names neither a revision nor a path is now git's `fatal: ambiguous argument` with exit 128 instead of exit 1 or a usage error. The removed walk scanned its result vector linearly per commit, so it was quadratic in history length. All 49 rev-list and log probe cases now match git.
- Fix: the long `status` format is git's, not house wording. It now prints git's section headers and per-section hints (`Changes to be committed:` with the `git rm --cached`/`git restore --staged` variants, `Changes not staged for commit:` switching to the `git add/rm` hint when a deletion is present, `Unmerged paths:`, `Untracked files:`, `Ignored files:`), tab-indented entries with the label padded to git's column (12, or 17 in the unmerged section), the `No commits yet` and `You have unmerged paths.` blocks, the tracking lines (`Your branch is up to date with 'origin/main'.`, ahead/behind/diverged with their hints), `HEAD detached at <abbrev>` at git's abbreviation length rather than a fixed 12, `-uno`'s `Untracked files not listed` line, and every closing summary variant -- including `nothing to commit, working tree clean`, which read `nothing to save, working tree clean`. Verified against git across clean, initial, staged, unstaged, untracked, ignored, deleted, renamed, conflicted, detached and all four tracking states.
- Fix: `tag` accepts git's own grammar, parsed in one pass instead of a chain of fixed argument shapes. `tag NAME [REV]` creates, `-a`/`-s`/`-u KEY` annotate or sign, `-m MSG` implies `-a`, `-f` moves an existing tag, `-d` deletes and `-v` verifies; listing gained `-n[NUM]`, `-l PATTERN`, `--contains`, `--merged`, `--points-at` and `--sort`, and these now combine, so `tag -n --contains REV` and `tag -l 'v1.*' --sort=-refname` work as they do in git. Creating is silent, `-f` reports `Updated tag` only when the ref actually moves, and `tag -d` reports git's `Deleted tag 'x' (was <abbrev>)` line instead of the full object id. Previously every git spelling exited 2 with a usage error. All 32 probe cases now match git.
- Fix: `rev-parse` accepts the query flags scripts depend on: `--symbolic-full-name` (the fully qualified ref a name stands for), `--verify` and `--quiet`/`-q` (exit 1 without a message when a revision does not resolve, where version previously exited 2 with a usage error), `--all`, `--revs-only`, `--show-prefix`, `--absolute-git-dir` and `--is-bare-repository`. All 20 probe cases now match git.
- Add: `tools/parity/compare.sh` -- runs a list of argument lines through both `git` and `version` in freshly built, identical fixtures and reports where stdout, exit status or stderr-emptiness diverge. It isolates `HOME` (the developer's `~/.gitconfig` sets `diff.algorithm=histogram`, which has faked a divergence before) and canonicalises the two fixture paths so absolute paths do not differ spuriously.
- Fix: `status` accepts the flags git's scripted callers rely on. `--branch`/`-b` is now a modifier that combines with `--short` or `--porcelain` (it was a mutually exclusive format, so `status --short --branch` failed as a "duplicate status mode option" and no `## branch` header was reachable from the short form); `--long` and `--porcelain=v1` are accepted; `-z` terminates machine-readable records with NUL; and `-uno`/`--untracked-files=no` omits untracked files. Every machine-readable form now matches git byte for byte.
- Fix: `filter-branch --subdirectory-filter` no longer keeps commits that never touched the subdirectory. git remaps such a commit onto its ancestor, so the rewritten history contains no commit that changed nothing; version kept them, leaving empty commits behind. The filter now implies `--prune-empty`, which was already implemented but never switched on for it.
- Fix: `gc` writes `objects/info/packs` and a commit-graph, as git does.
- Fix: `bundle create --all` bundles every ref and HEAD, as git does, instead of only the branches -- tags were silently left out, so a bundle made with `--all` restored a repository without them. `bundle create` also accepts a full refname (`refs/tags/x`) as an operand rather than rejecting it as unrecognized.
- Fix: `multi-pack-index verify` rejects a structurally corrupt index (an out-of-order object fanout) instead of accepting anything whose trailing checksum happens to match.
- Fix: `lfs pointer` no longer appends a trailing newline the pointer must not have. The output went through `Ada.Text_IO.Put`, which leaves the runtime mid-line so GNAT terminates it at exit; a pointer's object id is computed over its exact bytes, so the extra byte made every pointer version produced differ from git-lfs's. Now byte-identical.
- Fix: `lfs status --porcelain` prints git-lfs's short form instead of the human listing -- the flag was accepted and ignored. The column follows git-lfs's rule, which is not git's: an add or delete is reported in the first column and a modification in the second, whether staged or not.
- Fix: `hook run` runs hooks declared in configuration. Every `hook.<id>.command` whose `hook.<id>.event` names the hook now runs, in configuration order, before the one in `.git/hooks`, and a failing one stops the run and supplies the exit status -- previously only the hookdir hook ran, so a configured hook was silently ignored and `hook run` reported the hook as missing.
- Fix: `mktree` rejects a blank line outside batch mode with git's `fatal: input format error: (blank line only valid in batch mode)` and exit 128, instead of ignoring it and writing a tree.
...
- Fix: `for-each-repo` stops at the first repository whose command fails, as git does.
- Fix: `for-each-repo` stops at the first repository whose command fails and exits with that command's status, as git does, instead of continuing through the remaining repositories.
- Fix: `check-attr` splits its operands as git does. Without `--`, the first operand is the single attribute name and everything after it is a path -- version instead read attribute names until one happened to name an existing file, so `check-attr text binary a.txt` queried two attributes on one path where git queries one attribute on two paths, and a non-existent path was silently taken for an attribute. With `--`, every operand before it is still an attribute.
- Fix: `difftool` compares the index against the working tree, and under `--cached` HEAD against the index, as git does. Both modes previously compared HEAD against the working tree, so for a file that was staged and then modified again the tool was handed the wrong pair of sides -- `--cached` showed the unstaged edit it is meant to exclude.
- Fix: `mailinfo` decodes RFC 2047 encoded-words in `Author`/`Subject`, strips every leading bracket group and a leading `Re:`, and omits the `Date:` line entirely when the mail carries no date instead of printing an empty one.
- Fix: `index-pack -o <file>` writes only the index it was asked for, instead of also leaving a stray `<pack>.idx` beside the pack; and `index-pack --stdin` reports `pack<TAB><checksum>` as git does, rather than a bare checksum (the caller cannot learn the pack's name any other way). Note: git also writes a `.rev` reverse index, which version still does not; git rebuilds it in memory when absent.
- Fix: a plain `fetch <remote>` now records `.git/FETCH_HEAD`, one line per fetched branch, with the current branch's upstream marked for-merge and listed first, exactly as git writes it. Only the explicit-ref form (`fetch <remote> <ref>`) recorded it before, so anything reading FETCH_HEAD -- `fmt-merge-msg`, scripted merges, `merge FETCH_HEAD` -- found nothing after an ordinary fetch.
- Fix: `merge-index` exit statuses match git. It dies with `fatal: merge program failed` and exit 128 at the first failing merge program, but under `-q` exits 1 silently; with `-o` it attempts every path and then dies, or under `-q` exits with the number of failures. A named path that is not unmerged is now `fatal: git merge-index: <path> not in the cache` with exit 128, instead of silently succeeding.
- Fix: `credential fill` refuses a credential with no host (`fatal: refusing to work with credential missing host field`) or no protocol, with git's exit 128, instead of exiting 0 having echoed the partial record back. git checks host first, so an input with neither reports the missing host.
- Fix: `ls-remote --refs` now suppresses the `HEAD` line and the peeled `refs/tags/<t>^{}` entries, as git does; the flag was parsed and ignored.
- Fix: `ls-remote` matches its ref pattern operands. They were collected and never used, so `ls-remote <repo> 'v1*'` printed nothing. Patterns follow git's rule -- each becomes `*/<pattern>` wildmatched against `/<refname>` with `*` crossing slashes, against the full name, so a pattern that ends before `^{}` excludes the peeled line.
- Fix: `mailsplit` handles a Maildir. A directory operand was passed to the mbox reader, which exhausted the heap (`STORAGE_ERROR`, exit 141); it now reads `cur/` then `new/` as git does, skipping dotfiles and ordering names naturally (so `2` precedes `10`). Also: every CRLF line ending becomes LF unless `--keep-cr` (git's default, previously the CR was kept); an empty mailbox is now git's `error: empty mbox: '<file>'` + `error: cannot split patches from <file>` with exit 1 instead of reporting 0 and succeeding; and a mailbox whose first line is not a `From ` line is now `corrupt mailbox` with exit 1 unless `-b` is given.
- Fix: `format-patch` now emits an appliable `GIT binary patch` for a binary change (git's `--binary`, which format-patch implies) instead of an unappliable `Binary files ... differ` line, together with git's full (unabbreviated) index line. The compressed payload is not byte-identical to git's -- a zlib stream is not canonical -- but `git am`/`git apply` reproduce the exact blob.
- Fix: `diff --stat` scales the `+`/`-` bar graph to git's column budget and elides over-long names as `...`, matching git's `show_stats`; previously one mark was printed per changed line, so a large diff produced much wider output than git.
- Fix: `diff` now detects renames by default, as git does: a deleted path paired with a created path is reported as one rename, with git's `similarity index NN%`/`rename from`/`rename to` block (no index line or hunks when the content is unchanged), git's `a => b` and brace-compressed `d/{a => b}` names in `--stat`, and `R<nnn>` plus both paths in `--name-status`. The similarity number is git's own (a port of git's `diffcore-delta.c`), not an approximation. New flags `-M`/`-M<n>`/`--find-renames[=<n>]` and `--no-renames`, and the `diff.renames` configuration, all match git.
- Fix: `diff --stat`/`--name-status`/`--name-only` now report a mode-only change (identical content, changed file mode) instead of omitting the file entirely; `--stat` also prints git's ` 0 insertions(+), 0 deletions(-)` footer for a change with neither.
- Fix: `format-patch` now selects commits with a real rev-list walk instead of the rebase replay list: `<A>..<B>`, a bare `<since>` and `-<n>` all traverse every parent of a merge in committer-date order, so a range spanning a merge no longer fails with `error: rebase of merge commits not supported`, and `-<n>` picks the same commits git does on branched history. Merge commits are skipped, as git skips them.
- Fix: `format-patch` now matches git's mbox output byte-for-byte (apart from the trailer's git version number, which is git's own): it emits the diffstat + summary block after the `---` line, accepts the `-<n>` form (the last n non-merge commits, root-inclusive when the history is shorter than n), separates consecutive patches with a blank line (none after the last), and no longer appends a spurious extra trailing newline (the `--stdout` stream now goes through `Version.Console`). Previously it omitted the diffstat, rejected `-<n>` (`unknown format-patch argument: -1`), and emitted an extra blank line at the end.
...
- Fix: `cat-file --batch`/`--batch-check` now honour `-Z` for output framing: every record (the info line and, for `--batch`, the content record) is terminated with NUL instead of a newline, matching git. `-z` still reads NUL-separated input but keeps newline-terminated output, as git does. Previously both flags left the output newline-terminated.
- Fix: `patch-id --stable` now computes git's order-independent id -- each file's diff is hashed separately and the 20-byte digests are summed (with carry) into the result, so reordering the files in the patch does not change the id -- instead of ignoring the flag and always emitting the `--unstable` id. `--unstable` (the default) is unchanged.
- Fix: `hash-object` with several file operands now hashes every file, printing one object id per line (and, with `-w`, writing every blob), and dies (`fatal: could not open '<file>' for reading: No such file or directory`, exit 128) at the first unopenable file after emitting the ids of the files before it -- matching git. Previously it hashed only the first operand and ignored the rest.
- Fix: `mv` now accepts a directory as its source -- it renames every tracked file under the directory (the index is byte-identical to git, both for a plain rename and for a move into an existing directory) and removes the emptied source directory, instead of failing with `not under version control`. (An untracked file inside the moved directory is left in place rather than carried along, unlike git's whole-directory rename.)
- Fix: `check-ref-format --normalize` now rejects a refname with a trailing slash (an empty final component is invalid, exit 1) instead of silently dropping the slash and accepting it; leading and repeated slashes still normalize as before.
- Fix: `check-ref-format --branch` now rejects `HEAD` and a leading `-` as invalid branch names (`fatal: '<name>' is not a valid branch name`, exit 128) while still accepting `@` and ordinary names, matching git; previously they were accepted (exit 0).
- Fix: `diff-tree <merge-commit>` now prints nothing by default (a merge needs `-m`/`-c`/`--cc`, which version does not implement), matching git, instead of printing the commit id and a first-parent diff.
- Fix: `check-ignore` now validates its path operands as git does: an empty string is a fatal invalid pathspec (`fatal: empty string is not a valid pathspec. ...`), and a path resolving outside the repository is fatal (`fatal: <p>: '<p>' is outside repository at '<root>'`), both exit 128, instead of silently treating them as not-ignored (exit 1).
- Fix: `hash-object <file>` on a file that cannot be opened now dies with git's `fatal: could not open '<file>' for reading: No such file or directory` and exit 128, instead of the generic `error:` and exit 1.
- Fix: `get-tar-commit-id` on input that is not a well-formed archive now dies with git's `fatal: git get-tar-commit-id: EOF before reading tar header: No such file or directory` and exit 128, instead of `error:` and exit 1.
- Fix: `init` is now idempotent -- re-running it on an existing repository reinitialises it (exit 0, ensuring the directory structure, preserving HEAD/config) as git does, instead of failing with exit 1.
- Fix: `merge-tree --write-tree` now refuses histories with no common ancestor (`fatal: refusing to merge unrelated histories`, exit 128) and honours `--allow-unrelated-histories` to merge against an empty base, matching git; previously the flag was ignored and the merge errored with `no merge base found`.
- Fix: `bisect` now runs git's sanity check that every good rev is an ancestor of the bad rev; when it is not (good and bad likely swapped) it prints git's `Some good revs are not ancestors of the bad rev.` / `git bisect cannot work properly in this case.` / `Maybe you mistook good and bad revs?` and exits 1, instead of silently reporting a meaningless "first bad commit".
- Fix: `cat-file --batch`/`--batch-check` now honours `-z`/`-Z`, reading the request list as NUL-separated records instead of newline-separated, so NUL-delimited input is parsed correctly instead of being treated as one malformed object name.
- Fix: `diff-files` and `diff-index` (working-tree form) now report a mode-only change (chmod) and use the working file's on-disk mode in the raw record, instead of only detecting content changes and echoing the index mode. `diff-index`'s working-side object id is a zero id whenever the file is not fully up to date with the index (content or mode), matching git.
- Fix: `ls-files -m` now lists worktree-deleted tracked files as well as modified ones (git's `-m` is every path that differs from the index in the working tree), instead of omitting deletions.
- Fix: `diff` now shows a file-mode change (chmod) as git does -- `old mode`/`new mode` header lines -- whether the mode changed alone (pure header, no hunk) or alongside content (before the index line, which then drops its trailing mode), and for the working tree as well as `--cached`/committed diffs. The working-tree side now takes the file's on-disk mode instead of blindly reusing the index mode, so a chmod is no longer invisible.
- Fix: `grep` on a binary file now prints `Binary file <path> matches` once (never the raw NUL-containing line) in default/`-n` output, while `-c` and `-l` still count and list it, matching git.
- Fix: `count-objects -H`/`--human-readable` now formats the size as git does (`X.XX KiB`/`MiB`/`GiB`, or `<n> bytes`) instead of falling back to `<n> kilobytes`.

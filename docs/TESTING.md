...
Config tests freeze the `version config list`, `version config get KEY`, `version config set KEY VALUE`, and `version config unset KEY` contracts, including quoted subsection rendering such as `remote.origin.url`, deterministic `section.key=value` lines, direct value lookup, deterministic key creation/update, missing-key failure, selected-key removal, unrelated-entry preservation, no-mutation behavior for inspection commands, and rejection of a bare word like `keys`/`has` as a sectionless key (matching git).

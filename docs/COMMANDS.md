...
Syntax: `version shortlog [<options>] [<revision-range>] [[--] <path>...]`, or `version log --pretty=short | version shortlog [<options>]`.

Purpose: summarize history grouped by author, as `git shortlog` does -- a port of builtin/shortlog.c. Each group prints `Name (N):` and its subjects indented six spaces (the walk's order reversed, so oldest first), or with `-s`/`--summary` only `%6d\t<name>` counts; `-n`/`--numbered` sorts groups by descending count (stable, so equal counts stay in name order); `-e`/`--email` keys groups by `Name <mail>`; `.mailmap` always applies. `-c`/`--committer` and `--group=<author|committer|trailer:<key>|format:<fmt>|<fmt with %>>` (repeatable, combined; `--no-group` clears) pick the grouping -- a trailer group makes one record per matching trailer value (idents mapped through `.mailmap`, case-insensitive key), a format group one per expanded format, and a commit counted once per distinct key. Subjects are git's: leading whitespace and a leading `[PATCH ...]` dropped, the first paragraph folded to one line, `<none>` for an empty message; `--format=<fmt>`/`--pretty=format:<fmt>` (with `--date=<mode>`) replace the subject with the expanded format, while the named layouts keep the subject. `-w[<width>[,<indent1>[,<indent2>]]]` (default 76,6,9; `-w0` only indents) wraps each subject with git's word wrap, and rejects a width no larger than an indent. The revision arguments are the log walk's: ranges, `^rev`, `--all`/`--branches`/`--tags`/`--remotes`, `--no-merges`/`--merges`, `--first-parent`, `--min/max-parents`, `-<n>`/`--max-count`, `--skip`, `--since`/`--until`, `--author`, `--grep` (with `-i`, `-E`/`-F`/`-P`, `--all-match`, `--invert-grep`), `--reverse`, `--output=<file>`, and pathspecs after `--`; an operand that is neither is git's `fatal: ambiguous argument ...` die. With no revision and stdin not a terminal, the records are read from a `log` stream on stdin (`Author: `/`author ` -- or the committer lines under `-c` -- then the subject after the headers), which refuses trailer, format and multiple groups as git does.

### grep

Syntax: `version grep [<options>] [-e] <pattern> [<rev>...] [[--] <path>...]`.

Purpose: search tracked content for patterns, as `git grep` does -- the matcher and printer are a port of git's grep.c, the argument handling of builtin/grep.c. Patterns come from the first non-option argument or from `-e <pattern>` (repeatable) and `-f <file>` (one per line, empty lines skipped, `-` for stdin), and combine into git's expression: adjacent patterns are OR'ed, `--and`, `--not`, `(` and `)` build the rest (`--or` is the no-op it is in git; `--all-match` requires every top-level OR term to hit somewhere in the file). Patterns are basic regexps (`-G`, the default; `grep.extendedRegexp`/`grep.patternType` change it), extended (`-E`), fixed strings (`-F`) or perl-style (`-P`); `-i` ignores case, `-w` matches whole words with git's rescan past a partial match, `-v` inverts (and turns `-o` off). A rejected pattern dies with git's text (`fatal: -e option, '[': Invalid regular expression`, `In '<file>' at <n>, ...`).

Output follows git line for line: `path:text`, `-n` line numbers, `--column` the 1-based column of the first match (across `--and`/`--not` the way git chooses it), `-h`/`-H` hide/show names, `--full-name` root-relative names, `-l`/`--files-with-matches`, `-L`/`--files-without-match`, `-c` counts (files with none are not listed), `-o` only the matching parts (earliest, then longest, across patterns), `-q` status only, `-z` NUL after names, `-m <n>` matches per file, `--color[=<when>]` (`color.grep`, `color.grep.*` slots, `color.ui`), `--break` and `--heading`. Context: `-A`/`-B`/`-C <n>`, `-<n>`, `--context=<n>` with git's `--` separators between hunks and files, `-p`/`--show-function` (the enclosing function's line with `=`), `-W`/`--function-context` (the whole function, comments before it included), using git's default function rule (a line starting with a letter, `_` or `$`). Binary files (a NUL in the first 8000 bytes, or `-diff`/`diff` attributes) print `Binary file <name> matches` unless `-a`/`--text` or `-I`; `--textconv` runs the path's textconv driver. Names are C-quoted unless `-z`, relative to the directory the command ran in (which is also the default pathspec there).

Sources: the working files of the index by default (unmerged entries once, regular files only), the index blobs under `--cached`, one or more `<rev>` operands (commits, tags, trees, or `rev:path` blobs -- names prefixed `rev:`), the working tree plus untracked files under `--untracked`, or every file under `--no-index` (`--[no-]exclude-standard` chooses whether `.gitignore` applies, and is refused for tracked content). Revisions are read up to `--` and must all resolve when one is given (`fatal: unable to resolve revision: <x>`); without `--`, a name that is both a revision and a file is `ambiguous argument '<x>': both revision and filename`, the first non-revision starts the paths, and a path that does not exist is git's `ambiguous argument`/`no such path in the working tree` die (an argument starting with `-` after the pattern is `option '<x>' must come before non-option arguments`). Pathspecs take git's default wildcards (`*.c` reaches into subdirectories); `--max-depth=<n>` limits the depth below each pathspec, `-r` lifts it. `--cached` with revisions, and more than one of `--no-index`/`--untracked`/`--cached`, are refused as git refuses them; `-m 0` exits 1 at once. `--threads`, `--recurse-submodules`, `-O`/`--open-files-in-pager` and `--ext-grep` are accepted without effect. The exit status is 0 when anything matched, 1 otherwise.

### describe

Syntax: `version describe [--all] [--tags] [--contains] [--abbrev=<n>] [<commit-ish>...]`, `version describe [...] --dirty[=<mark>] [--broken[=<mark>]]`, `version describe <blob>`.

Purpose: name each commit-ish (default HEAD) relative to the nearest reachable tag, exactly as `git describe` does -- a port of builtin/describe.c. The tag itself when the commit is tagged, otherwise `<tag>-<N>-g<short>` where N counts the commits since the tag and `<short>` is the unique abbreviation (`--abbrev=<n>` at least n digits, minimum 4; `--abbrev=0`/`--no-abbrev` drops the suffix; `--long` keeps it even on an exact match, and refuses `--abbrev=0`). Only annotated tags count unless `--tags` (lightweight too) or `--all` (any ref, named with its namespace: `tags/v1`, `heads/main`, `remotes/origin/main`). Candidates are gathered in commit-date order up to `--candidates=<n>` (default 10, `--exact-match` is 0: the commit must be tagged), the nearest wins with ties to the first found, `--first-parent` follows only first parents, `--match <glob>` (repeatable; `--no-match` clears) keeps only matching names and `--exclude <glob>` drops them (under `--all` the globs apply to tags, heads and remotes). Two annotated tags on one commit are decided by the newer tagger date; a tag whose object carries another name prints that name, warns `tag '<ref>' is externally known as '<name>'`, and always gets the suffix. `--always` falls back to the abbreviated id; `--debug` reports the search on stderr (`describe <arg>`, the candidates with their kind and depth, `traversed N commits`, `found N tags; gave up search at <id>`). Without an operand `--dirty[=<mark>]` appends the mark (default `-dirty`) when the index or tracked files differ from HEAD; `--broken[=<mark>]` does the same check (its own mark is for a check that cannot run); either with an operand is `fatal: option '--dirty' and commit-ishes cannot be used together`. A blob operand (`HEAD:path`) is described as the first commit from HEAD's history (oldest first) whose tree holds it, then `:<path>`. `--contains` names the commit by the nearest ref that *contains* it instead, as `name-rev --tags --name-only --no-undefined` (with `--always`, `--all`, `--match`/`--exclude` passed along; `fatal: cannot describe '<id>'` when nothing does). Errors: `fatal: No names found, cannot describe anything.`, `fatal: No annotated tags can describe '<id>'.\nHowever, there were unannotated tags: try --tags.`, `fatal: No tags can describe '<id>'.\nTry --always, or create some tags.`, `fatal: no tag exactly matches '<id>'`, `fatal: Not a valid object name <arg>`, `fatal: <arg> is neither a commit nor blob`, `fatal: blob '<id>' not reachable from HEAD`.

### notes

Syntax: `version notes [--ref <notes-ref>] [list [<object>]]`, `version notes [--ref <notes-ref>] add [-f] [--allow-empty] [--[no-]separator|--separator=<paragraph-break>] [--[no-]stripspace] [-m <msg> | -F <file> | (-c | -C) <object>] [<object>] [-e]`, `... copy [-f] <from-object> <to-object>`, `... copy [-f] --stdin`, `... copy --for-rewrite=<cmd>`, `... append [--allow-empty] [--[no-]separator|--separator=<paragraph-break>] [--[no-]stripspace] [-m <msg> | -F <file> | (-c | -C) <object>] [<object>] [-e]`, `... edit [--allow-empty] [<object>]`, `... show [<object>]`, `... merge [-v | -q] [-s <strategy>] <notes-ref>`, `version notes merge --commit [-v | -q]`, `version notes merge --abort [-v | -q]`, `... remove [--ignore-missing] [--stdin] [<object>...]`, `... prune [-n] [-v]`, `... get-ref` -- git's option surface, a port of builtin/notes.c, notes.c and notes-merge.c.

Purpose: attach, show and manage text notes on objects (default HEAD; notes attach to the object a revision names directly, so a tag's note sits on the tag object). Notes live in a notes ref: `refs/notes/commits` unless `--ref <name>` (also `--ref=<name>`; a bare name means `refs/notes/<name>`, `notes/<name>` is completed to `refs/notes/<name>`), `$GIT_NOTES_REF` or `core.notesRef` selects another. Every writing subcommand refuses a ref outside `refs/notes/` (`fatal: refusing to <sub> notes in <ref> (outside of refs/notes/)`). The notes tree is written exactly as git writes it, including git's 2-hex-digit fanout once a tree grows large, and any fanout layout is read; every change is one commit on the notes ref (`Notes added by 'git notes <sub>'` / `Notes removed by 'git notes <sub>'`, reflogged as `notes: ...`), made only when the tree actually changed.

Bare `notes` lists, as git does. `list` prints `<note-blob> <object>` per note in object-id order, or just the blob id for one object (`error: no note found for object <id>.`, exit 1). `show` prints the note blob verbatim (an empty note prints nothing and succeeds). `get-ref` prints the notes ref in effect.

`add`, `append` and `edit` build the note from the message pieces in the order given: `-m <msg>` (also `--message=`), `-F <file>`/`--file=` (`-` for standard input), `-C <object>`/`--reuse-message=` (a blob's content verbatim; `fatal: cannot read note data from non-blob object '<x>'.`), `-c <object>`/`--reedit-message=` (the same, then opened in the editor). The pieces are joined by the separator -- a newline by default, `--separator=<text>` (newline-appended unless it ends in one), `--separator` alone a newline, `--no-separator` nothing -- and the accumulated text is stripspaced after each `-m`/`-F` piece (`--stripspace` after every piece, `--no-stripspace` never), exactly git's `concat_messages`. With no piece, or with `-e`/`--edit`, the editor (`GIT_EDITOR`, `core.editor`, `VISUAL`, `EDITOR`) opens `.git/NOTES_EDITMSG` seeded with the pieces so far (or the existing note for `edit`, and for `add -f`), followed by git's commented template (`# Write/edit the notes for the following object:` and `show --stat --no-notes` of the object); the result is stripspaced with comment lines dropped unless `--no-stripspace`. An empty result removes the note (`Removing note for object <id>`) rather than storing it, unless `--allow-empty`. `add` refuses to replace an existing note without `-f` when a message was given (`error: Cannot add notes. Found existing notes for object <id>. Use '-f' to overwrite existing notes`, exit 1), redirects to `edit` when none was, and reports `Overwriting existing notes for object <id>` on stderr under `-f`; `append` joins the new text to the existing note with the separator between (when both are non-empty); `edit` with `-m`/`-F`/`-c`/`-C` warns that those options are deprecated for it, as git does. Too many operands is a usage error.

`copy [-f] <from> [<to>]` (default `HEAD`) copies one note (`error: missing notes on source object <id>. Cannot copy.`; `error: Cannot copy notes. Found existing notes for object <id>. Use '-f' to overwrite existing notes`); `copy --stdin` reads `<from> <to>` lines (`fatal: malformed input line: '<line>'.`, `fatal: failed to resolve '<x>' as a valid ref.`, `error: failed to copy notes from '<from>' to '<to>'` for a refused overwrite, exit 1 with the rest still copied). `copy --for-rewrite=<cmd>` implies `--stdin` and rewrites the notes of every ref `notes.rewriteRef` names (globs allowed; a ref outside `refs/notes/` draws `warning: Refusing to rewrite notes in <ref> (outside of refs/notes/)`) when `notes.rewrite.<cmd>` is not false, combining with `notes.rewriteMode` (`overwrite`, `concatenate` -- the default, `cat_sort_uniq`, `ignore`; a bad value is reported and no combining happens); `GIT_NOTES_REWRITE_REF` (colon-separated) and `GIT_NOTES_REWRITE_MODE` take precedence over the config.

`remove [<object>...]` (default `HEAD`; `--stdin` adds one object per line, right-trimmed) reports `Removing note for object <x>` / `Object <x> has no note` on stderr per operand as written, `error: Failed to resolve '<x>' as a valid ref.` for an unresolvable one, and commits only when nothing failed (`--ignore-missing` makes a missing note not a failure). `prune` drops the notes whose object no longer exists; `-v` lists them, `-n` only lists.

`merge <notes-ref>` merges another notes ref (or commit) into the current one, as git's notes-merge.c: trivial cases (unborn side, `Already up to date.`, `Fast-forward`) just move the ref; otherwise each note the remote side changed against the merge base is adopted unless the local side changed it too, in which case the strategy decides -- `-s`/`--strategy` `manual` (the default), `ours`, `theirs`, `union`, `cat_sort_uniq`, or `notes.<short-ref>.mergeStrategy` then `notes.mergeStrategy` -- and a merge commit `Merged notes from <remote> into <local>` with both parents is made. Under `manual`, a conflicting note is written with conflict markers (or, for delete/modify, the surviving version) to `.git/NOTES_MERGE_WORKTREE/<object>`, git's `CONFLICT (content|add/add|delete/modify)` lines are printed, the partial result is parked in `NOTES_MERGE_PARTIAL`/`NOTES_MERGE_REF`, and the command exits 1 with `Automatic notes merge failed. Fix conflicts in .git/NOTES_MERGE_WORKTREE and commit the result with 'git notes merge --commit', or abort the merge with 'git notes merge --abort'.`; `merge --commit` then takes every file left in the worktree as the resolved note and finalizes the parked commit, `merge --abort` discards it. Starting another merge while one is parked is refused. `-v` (repeatable) and `-q` adjust git's verbosity (`Auto-merging notes for`, `Using local/remote notes for`, `Concatenating ...`, merge-base reports at `-vv`). `--commit`, `--abort` and `-s` cannot be mixed (usage error), and `merge` needs exactly one ref.

### blame

Syntax: `version blame [<options>] [<rev-opts>] [<rev>] [--] <file>` (also `version blame <file> <rev>`); `version annotate` is `blame -c`.

Purpose: show, for each line of FILE, the commit that introduced it, as `git blame` does -- the engine is a port of git's blame.c scoreboard. The final image (the working file by default; a revision's copy when one is named; `--contents <file>`, or `-` for stdin, for any text at all) starts as one suspect run on the tip; each commit hands the lines its parents already had back to them along git's own diff, keeps the rest as its own, and the commits are examined newest-first until every line has found its guilty commit. Merges are followed through **every** parent (`--first-parent` restricts to the first), the file is followed across renames (`--no-follow` stops that), and an uncommitted line is reported against the all-zero id as `Not Committed Yet` (`External file (--contents)`) at the current time. Root commits and the ancestors of a range's bottom (`A..B`, `^A`, and everything older than `--since=<date>`) are boundaries, marked `^` (`--root` blames a root outright, `-b` and `blame.blankBoundary` blank the id instead).

Line attribution switches: `-w` ignores whitespace when matching lines, `--diff-algorithm=<algo>`/`--minimal` and `--[no-]indent-heuristic` pick the diff used for the matching (`diff.algorithm` and `diff.indentHeuristic` apply), `-M[<score>]` credits lines moved within the file and `-C[<score>]` lines copied from other files (once: paths deleted in the same commit; `-C -C`: any path when the file was created; `-C -C -C`: any path in every commit; `--find-copies-harder` is `-C -C`), with git's alphanumeric-count scores (defaults 20 and 40). `--ignore-rev <rev>` and `--ignore-revs-file <file>` (also `blame.ignoreRevsFile`; an empty name clears the list) skip a commit's changes, re-attributing its lines to the nearest match in the parent with git's fingerprint heuristic and marking the leftovers `unblamable`/`ignored` (`?` and `*` in the id column, `unblamable`/`ignored` lines in porcelain, under `blame.markIgnoredLines`/`blame.markUnblamableLines`). `--reverse <bottom>..<tip>` walks forward instead, reporting the last commit each line survived to (a lone revision means `<rev>..HEAD`; `--reverse --first-parent` needs the tip named and a range along the first-parent chain). `-S <file>` reads grafts (`<commit> <parent>...` lines) that replace parents for the walk.

Output switches: `-s` suppresses the author and date, `-e`/`--show-email` (or `blame.showEmail`) prints the address, `-t` the raw timestamp, `--date=<mode>` (or `blame.date`; the default is `iso`) any of git's date modes padded to that mode's width, `-l` the full id, `--abbrev[=<n>]`/`--no-abbrev` the id width (git widens it by one for the mark column, never below four digits), `-f`/`--show-name` the filename column (on automatically once a line came from another path), `-n`/`--show-number` the original line number, `-c` git's `annotate` layout, `--score-debug` each run's score, `--color-lines` (or `blame.coloring=repeatedLines`, `color.blame.repeatedLines`) colours the metadata of a line repeating the previous commit and `--color-by-age` (or `blame.coloring=highlightRecent`, `color.blame.highlightRecent`) colours by author date. `-L <start>,<end>` limits the lines: absolute numbers, `,+<n>`/`,-<n>` relative to the start, `/<regex>/` (searched from the previous range's end; `^/<regex>/` from the top) and `:<funcname>` (a header line matching the regex through the line before the next header), given more than once for several ranges, which are merged and sorted. `--porcelain` emits git's machine format (a header per run, the commit block once per commit -- or per line under `--line-porcelain` -- with `boundary`/`previous`/`filename`), and `--incremental` the same records in the order the commits were found. `--show-stats` appends git's work counters. `--progress` is accepted (and refused with `--incremental`/porcelain, as in git); `--encoding=` is accepted. Errors follow git: `fatal: no such path '<file>' in HEAD`, `fatal: no such path <file> in <rev>`, `fatal: bad revision '<x>'`, `fatal: file <file> has only N lines`, `fatal: -L parameter '<re>' starting at line N: No match`, `fatal: cannot find revision <x> to ignore`, and a malformed `-L` is a usage error.
...
Purpose: show commit history from HEAD or the given revisions, byte-for-byte as git renders it. Author dates render in git's default format (`Www Mmm D HH:MM:SS YYYY ±HHMM`, in the commit's timezone) unless `--date` says otherwise. The default layout expands tabs in the message to 8 columns (medium/full/fuller), applies `.mailmap` to the identities (`log.mailmap`, on by default), and shows the commit's note. `--oneline` prints one compact `<short-id> <subject>` line per commit (short id abbreviated to git's shortest-unique length, 7-char floor, or `--abbrev=<n>`); `--no-abbrev-commit` spells ids out and `--abbrev-commit` shortens the `commit` line of the full layouts. `--graph` draws git's ASCII commit graph and walks in topological order. `--decorate` lists the refs at each commit (`HEAD -> main, tag: v1, origin/main`; with `--decorate-refs`/`-exclude` every ref namespace takes part, as in git). `--source` names the ref each commit was reached from; `--left-right`/`--cherry-mark`/`--boundary` mark commits `<`/`>`/`=`/`+`/`-` in every layout. `-g` walks the reflog of the first revision (HEAD by default) with git's `Reflog: HEAD@{n} (ident)` / `Reflog message:` lines (or the `<id> HEAD@{n}: message` oneline), `HEAD@{<date>}` selectors under an explicit `--date`, `--grep-reflog` filtering and `%gd`/`%gD`/`%gs`/`%gn`/`%ge` placeholders. `--stdin` reads revisions (and, after a `--`, paths) from standard input; `--merge` shows `HEAD...MERGE_HEAD` limited to the conflicted paths. A commit note is shown by default but suppressed once an explicit `--pretty`/`--format` is given, unless `--notes` forces it back on; `--notes=<ref>` adds another notes ref (replacing the standard one unless `--notes` is also given) and git's `warning: notes ref ... is invalid` reports a missing one. `--follow <path>` (exactly one pathspec) continues a file's history across renames. Revision selection is the same as `rev-list`'s and shares its implementation; paths after `--` limit the history with git's default simplification, `--full-history`/`--sparse` relax it (`--simplify-merges`/`--show-pulls` are taken as `--full-history` in topological order: exact without a path limit, approximate with one), `--ancestry-path` keeps only the commits between the range's ends, `--simplify-by-decoration` only the decorated ones. `-m` shows a merge once per parent (`(from <parent>)`) with that parent's diff; `--diff-merges=first-parent` diffs it against its first parent; `-c`/`--cc` imply `-p` and render git's combined diff (see `show`); `--dd` is, as in git, the first-parent diff with `-p`; `--remerge-diff` is taken as `--cc`. `--line-prefix` prefixes every output line, `--output` writes to a file, `-z` separates records with NUL, `--log-size` prints git's `log size <n>` line.

### show

Syntax: `version show [OPTIONS] [<object>...] [--] [PATH...]` where an object is a commit, tag, tree, blob or `REV:PATH`; also every `log` form (`version show HEAD~3..HEAD`, `--do-walk`, `-<n>`) since `show` is `log` with git's show defaults.

Options: the whole `log` surface (below) applies -- selection, filters, layouts, dates, decorations, `--source`, the marks, notes, mailmap, `-z`, `--line-prefix`, `--output`, the pretty forms -- plus every `diff` switch. show's defaults differ from log's as in git: no history walk (each operand is shown on its own, in the order given; a range, `^rev`, `rev^!`, `-<n>`/`--max-count` or `--do-walk` turn the walk back on), the patch on unless a summary format (`--stat`, `--numstat`, `--shortstat`, `--summary`, `--name-only`, `--name-status`, `--raw`, `--compact-summary`) or `-s`/`--quiet` is given, and a merge shown as a **dense combined diff** (`--cc`; `-c` for the full one, `-m` one diff per parent, `--first-parent`/`--diff-merges=first-parent`/`--dd` against the first parent, `--diff-merges=off` none).

Purpose: show one or more objects byte-for-byte as git renders them. A commit gets the log header (all layouts and header switches apply) followed by its diff; `REV:PATH` prints a blob verbatim or git's `tree REV:PATH` listing for a directory; an annotated tag prints its `tag <name>` block (the tagger line per layout, none under `--oneline`) and then whatever it points at; a tree its listing; a blob its contents. Consecutive objects are blank-separated as git does (a blob neither gets nor counts as a separator; oneline and `tformat:` layouts run together). An operand that is no object but exists on disk limits the diffs, an option after such an operand is refused (`option '--stat' must come before non-option arguments`), and a missing `REV:PATH` dies with `fatal: path '<p>' does not exist in '<rev>'`. `--all`, `--branches[=<pat>]`, `--tags[=<pat>]`, `--remotes[=<pat>]`, `--glob=`, `--exclude=` and `--bisect` add the refs they name as objects (tags as tag objects, in ref order, `HEAD` last for `--all`), a commit reached twice is shown once, and the walk filters (`--grep`, `--author`, `--no-merges`, `--min/max-parents`, `--since/--until`, `--skip`, `-S`/`-G`) still apply. `--graph` without a walk is git's fatal. `--relative`, `-R`, `--check`, `--quiet`, `-I` and `--output` follow git with combined diffs (`--check` and `--quiet` print none; `-I` and `--output` are fatal on a merge; the `Binary files differ` line stays unprefixed under `--line-prefix`).

**Combined diffs** (`Version.Combine_Diff`, a port of git's combine-diff.c) render a merge's `diff --cc <path>` / `diff --combined <path>` blocks: paths where the result differs from at least one parent (a deleted path only when every parent had it), the `index p1,p2..r` line (unique abbreviation from 7, full under `--full-index` only), git's `new file mode` / `[deleted file ]mode p1,p2..r` lines, `@@@ -a,b -c,d +e,f @@@` hunks with one marker column per parent (`+` where that parent lacks the line, `-` lines the result dropped, coalesced across parents), git's context/interhunk joining, the dense rule that drops hunks changed against only one parent or taken wholesale from one parent, the 80-byte funcname copied up to (excluding) its last non-blank character, git's `-U0` unsigned wraparound in the parent counts, whitespace and algorithm switches, and `Binary files differ`. The summary formats of a merge under `-c`/`--cc` follow git: `--stat`/`--numstat`/`--shortstat`/`--summary` diff against the first parent, `--raw`/`--name-only`/`--name-status` list the combined path set (`::<modes> <ids> <statuses>\t<path>`), and the patch follows after a blank line when that set is non-empty; a pickaxe (`-S`/`-G`, `--pickaxe-regex`) keeps only the paths whose change against every parent matches and never drops a merge shown this way. `log -c/--cc` and `diff-tree -c/--cc` (raw listing, `--cc` implying `-p`) use the same engine. Not rendered: `--remerge-diff` (shown as `--cc`), `-R` on a combined diff, `--dirstat`.

`log` and `show` take `diff`'s hunk, whitespace, algorithm and presentation switches (`-w`, `-b`, `--ignore-space-at-eol`, `--ignore-cr-at-eol`, `--ignore-blank-lines`, `-I`, `-W`, `--inter-hunk-context=`, `--patience`/`--histogram`/`--minimal`/`--diff-algorithm=`, `--[no-]indent-heuristic`, `-R`, `--relative[=<path>]`, `--check`, `-O<orderfile>`, `--full-index`, `--textconv`/`--no-textconv`, `--submodule[=...]`, `--ws-error-highlight=`), applied to every `--stat`/`-p` rendering; a commit whose diff folds away gets no separator or diff, as in git. `--color` is not yet taken by `log`/`show` (the commit headers are not painted).
...
Syntax (git's grammar, a port of builtin/branch.c): `version branch [<options>] [-r | -a] [--merged [<commit>]] [--no-merged [<commit>]]`, `version branch [<options>] [-f] [--recurse-submodules] <branch-name> [<start-point>]`, `version branch [<options>] [-l] [<pattern>...]`, `version branch [<options>] [-r] (-d | -D) <branch-name>...`, `version branch [<options>] (-m | -M) [<old-branch>] <new-branch>`, `version branch [<options>] (-c | -C) [<old-branch>] <new-branch>`, `version branch [<options>] [-r | -a] [--points-at <object>]`, `version branch [<options>] [-r | -a] [--format <format>]`, `version branch (-u | --set-upstream-to=)<upstream> [<branch>]`, `version branch --unset-upstream [<branch>]`, `version branch --show-current`, `version branch --edit-description [<branch>]`. This CLI's word spellings remain: `version branch list [--verbose | --contains REV | --merged [BRANCH] | --no-merged [BRANCH]]`, `current`, `exists NAME`, `resolve NAME`, `upstream [BRANCH]`, `contains REV`, `merged [BRANCH]`, `unmerged [BRANCH]`, `create NAME`, `switch NAME`, `rename [OLD] NEW`, `delete [--force] NAME`, `integrate NAME | --finalize | --abort`, `finalize`, `set-upstream BRANCH REMOTE REMOTE_BRANCH`, `unset-upstream BRANCH`, `ahead-behind BRANCH`, `update NAME`.

Purpose: inspect and manage branches, byte for byte as git does. Options follow git's parse-options rules (bundled `-avv`, `--opt=value` or `--opt value`, `--no-` forms, `-t[=direct|inherit]` taking only an attached value, the commit filters `--contains`/`--with`, `--no-contains`/`--without`, `--merged`, `--no-merged` defaulting to `HEAD` only when last). Exactly one action may be given: `-d`/`-D`, `--show-current`, a listing (`-l`, no operand, or any filter), `--edit-description`, `-m`/`-M`, `-c`/`-C`, `--set-upstream-to`, `--unset-upstream` -- two of them are a usage error; otherwise one or two operands create. `-f` upgrades `-d`/`-m`/`-c` to their forcing forms.

Listing prints git's layouts through the `for-each-ref` engine: `  name` / `* name` for the checked-out branch / `+ name` for one checked out in another worktree, `remotes/` prefixes under `-a`, `origin/HEAD -> origin/main` for symrefs, and a detached HEAD as `* (HEAD detached at <abbrev>)` (or `detached from`, `no branch, rebasing <b>`, `no branch, bisect started on <b>`) sorted first. `-v` aligns the names and adds the abbreviated id (`--abbrev=<n>`, `--no-abbrev`), `[ahead N, behind M]`/`[gone]` against the upstream, and the subject; `-vv` names the upstream (`[origin/main: ahead 1]`) and the other worktree's path. Patterns are wildmatched against the short names (`*` crossing `/`; `-i` folds case in patterns and sorting), `--sort=<key>` is repeatable (last key primary; `branch.sort`; `--no-sort`), `--format=<format>` any `for-each-ref` template, `--omit-empty` drops empty lines, `--color[=<when>]` (`color.branch`, `color.branch.<slot>` for `current`/`local`/`remote`/`upstream`/`worktree`/`plain`/`reset`) colours the built-in layouts under `always`, and `--column[=<options>]`/`--no-column` (`column.branch`/`column.ui`) lays the names out in columns (`fatal: options '--column' and '--verbose' cannot be used together`). A branch on a non-commit never passes a commit filter; `--points-at` matches the ref's object or its peel.

Creating: `version branch <name> [<start-point>]` (default `HEAD`) refuses a bad name (`fatal: '<x>' is not a valid branch name` + git's `advice.refSyntax` hints), an existing one without `-f` (`fatal: a branch named '<x>' already exists`; `-f` moves it unless a worktree has it checked out), a bad start (`fatal: not a valid object name: '<x>'`, `fatal: not a valid branch point: '<x>'`), `-a`/`-r` with a name, `--set-upstream` (no longer supported), and `--recurse-submodules` without `submodule.propagateBranches`; it writes the reflog entry `branch: Created from <start>` (`branch: Reset to <start>` under `-f`; always in a non-bare repository, `--create-reflog` forces it). Tracking follows `branch.autoSetupMerge` (`true`: from a remote-tracking start; `always`; `false`; `inherit`; `simple`), `-t`/`--track[=direct]` (the start must be a branch: `fatal: cannot set up tracking information; starting point '<x>' is not a branch`), `--track=inherit` (copies the start branch's upstream, warning `asked to inherit tracking from '<b>', but no remote is set` when it has none) or `--no-track`; the remote and source branch come from the remote's fetch refspecs (a local branch tracks through the `.` remote), `branch.autoSetupRebase` adds `rebase = true`, and git's `branch '<x>' set up to track '<y>'[ by rebasing].` line is printed unless `-q`.

`-u <upstream>`/`--set-upstream-to=<upstream> [<branch>]` sets the upstream (dwim of `<remote>/<branch>`, a local branch, or a full ref; `fatal: the requested upstream branch '<x>' does not exist` + git's `advice.setUpstreamFailure` hints; `fatal: branch '<x>' does not exist`, `no commit on branch '<x>' yet`, `could not set upstream of HEAD to <x> when it does not point to any branch`, `too many arguments to set new upstream`); `--unset-upstream [<branch>]` drops it (`fatal: branch '<x>' has no upstream information`). `--show-current` prints the checked-out branch (nothing when detached). `--edit-description [<branch>]` opens `.git/EDIT_DESCRIPTION` with git's commented template and stores `branch.<x>.description` (multi-line values are escaped as git escapes them; an empty result removes it; `error: no branch named '<x>'` / `no commit on branch '<x>' yet`, `fatal: cannot give description to detached HEAD`, `cannot edit description of more than one branch`).

`-m`/`-M [<old>] <new>` renames (the current branch when one operand; `fatal: cannot rename the current branch while not on any`, `no branch named '<x>'`, `a branch named '<x>' already exists` without `-M`, `too many arguments for a rename operation`, `branch name required`): the reflog moves with the branch and records `Branch: renamed refs/heads/<old> to refs/heads/<new>` (HEAD's reflog too, as git's files backend does), HEAD is repointed, and the `branch.<old>` config section is renamed. `-c`/`-C` copies likewise (`Branch: copied ...`, the reflog and config section duplicated). `-d`/`-D` deletes: `-d` refuses a branch not merged into its upstream (or HEAD without one) with `error: the branch '<x>' is not fully merged` and the `advice.forceDeleteBranch` hint, printing git's reminders when upstream and HEAD disagree; a branch checked out in a worktree is `error: cannot delete branch '<x>' used by worktree at '<path>'`; missing ones are `error: branch '<x>' not found` (with `Did you forget --remote?` when a remote-tracking branch of that name exists) or `error: remote-tracking branch '<x>' not found` under `-r` (which implies force); every deletable branch is deleted and reported as `Deleted [remote-tracking] branch <x> (was <abbrev>).` unless `-q`, its reflog and config section removed; exit 1 when any failed; `-a` with `-d` is refused; no operand is `fatal: branch name required`.
...
Syntax: `version tag [-a | -s | -u <key-id>] [-f] [-m <msg> | -F <file>] [-e] [(--trailer <token>[(=|:)<value>])...] [--cleanup=<mode>] [--create-reflog] <tagname> [<commit> | <object>]`, `version tag -d <tagname>...`, `version tag [-n[<num>]] -l [--contains <commit>] [--no-contains <commit>] [--points-at <object>] [--column[=<options>] | --no-column] [--sort=<key>] [--format=<format>] [--merged <commit>] [--no-merged <commit>] [-i] [--omit-empty] [--color[=<when>]] [<pattern>...]`, `version tag -v [--format=<format>] <tagname>...` -- git's option surface, a port of builtin/tag.c -- plus this CLI's word spellings: `version tag create NAME [REV]`, `version tag create -a NAME [REV] -m MESSAGE`, `version tag delete NAME`, `version tag remove NAME`, `version tag rename OLD NEW`, `version tag list [...]`, `version tag exists NAME`, `version tag resolve NAME`, `version tag peel NAME`, `version tag show NAME`.

Purpose: manage lightweight and annotated tags. Options follow git's parse-options rules: bundled short flags (`-am msg`), `--opt=value` or `--opt value`, `--no-` negations, `-n[<num>]` taking only an attached number, and the commit filters (`--contains`/`--with`, `--no-contains`/`--without`, `--merged`, `--no-merged`, `--points-at`) defaulting to `HEAD` only when they are the last argument. The mode is git's: `-l`, `-d` and `-v` are exclusive (`error: options '-l' and '-d' cannot be used together`), no operand lists, a list filter or `-n` lists, otherwise the operands create; creation switches with a list/delete/verify mode are a usage error, and a filter with `-d`/`-v` is `fatal: the '<opt>' option is only allowed in list mode`.

Creating: `version tag <name> [<object>]` (default `HEAD`; any object -- a blob or tree gets a lightweight tag on it) refuses an existing tag without `-f` (`fatal: tag '<name>' already exists`), a bad name (`fatal: '<name>' is not a valid tag name.`, a leading `-` included), an unresolvable object (`fatal: Failed to resolve '<x>' as a valid ref.`) and a third operand (`fatal: too many arguments`); `-f` reports `Updated tag '<name>' (was <abbrev>)` only when the target changed. A tag *object* is made by `-a`, `-s`/`-u <key>` (also `tag.gpgSign`; `tag.forceSignAnnotated` signs a `-m`-only tag), `-m <msg>` (repeatable, paragraphs joined by a blank line), `-F <file>` (`-` for stdin; `fatal: options '-F' and '-m' cannot be used together`), `-e`, or `--trailer`. Without a message, or with `-e`, the editor opens `.git/TAG_EDITMSG` seeded with the message so far, the previous tag's message under `-f`, or git's commented `Write a message for tag:` template; `--trailer <token>[(=|:)<value>]` runs the file through `interpret-trailers` (trailers land before a trailing comment block, as git's do). `--cleanup=strip` (default: whitespace and `#` lines removed), `whitespace` (comments kept) or `verbatim` (the bytes as given, stored without a final newline when there is none); an empty result is `fatal: no tag message?` unless `-m ''`/`-F` gave it explicitly, and an editor failure prints `Please supply the message using either -m or -F option.` (exit 1). Tagging a tag object prints git's `hint: You have created a nested tag ...` advice (`advice.nestedTag`). `--create-reflog` writes the tag's reflog entry `tag: tagging <abbrev> (<subject>, <date>)` (`GIT_REFLOG_ACTION` replaces the prefix). A signing failure reports `error: gpg failed to sign the data:` with gpg's output, `error: unable to sign the tag` and `The tag message has been left in .git/TAG_EDITMSG`.

Listing: `version tag [-l] [<pattern>...]` prints the tag names (each pattern a wildmatch of the whole short name, `*` crossing `/`; `-i` folds case in patterns and sort keys), `-n[<num>]` git's `%(align:15)%(refname:lstrip=2)%(end) %(contents:lines=<num>)` layout (the first lines of the annotation, or of the commit message for a lightweight tag, continuation lines indented four spaces), `--format=<format>` any `for-each-ref` template (`-n` is then ignored), `--sort=<key>` repeatable (the last key is the primary one, `-` prefix descending, `version:refname`/`v:refname`, date and size keys numeric; `tag.sort` supplies defaults, `--no-sort` clears them), `--contains`/`--no-contains` (tags whose commit contains, or does not contain, any of the given commits; a tag on a non-commit never matches), `--merged`/`--no-merged` (tags whose commit is reachable from any of the given commits, or from none), `--points-at <object>` (the ref's own object or its peeled target; repeatable), `--omit-empty`, `--color[=<when>]` (`%(color:...)` atoms expand under `always`), and `--column[=<options>]`/`--no-column` with git's column layout (`always`, `never`, `auto`, `column`, `row`, `plain`, `dense`; `column.tag`/`column.ui`; width from `$COLUMNS` or 80; `fatal: options '--column' and '-n' cannot be used together`). A bad filter argument is git's `error: malformed object name ...`/`error: no such commit ...` usage error.

`-d <tagname>...` deletes and reports `Deleted tag '<name>' (was <abbrev>)` per tag; a missing one is `error: tag '<name>' not found.` and exit 1, the rest still deleted. `-v <tagname>...` prints the tag object and the verification verdict (`error: no signature found` for an unsigned tag, `error: <name>: cannot verify a non-tag object of type <type>.` for a lightweight one); `--format` prints the template for a verified tag instead of the object.

The word spellings remain: `version tag create NAME [REV]` (lightweight) and `create -a NAME [REV] -m MESSAGE` (annotated) create; `list` is `-l`, `delete`/`remove` are `-d`; `rename OLD NEW` moves a tag ref without rewriting the object; `exists NAME` reports by exit status; `resolve NAME` prints the id stored in the ref, `peel NAME` the peeled target, `show NAME` stable tag details. Note that git would take these words as tag names.
...
Syntax: `version stash list [<log-options>]`, `version stash show [-u | --include-untracked | --only-untracked] [<diff-options>] [<stash>]`, `version stash drop [-q | --quiet] [<stash>]`, `version stash pop [--index] [-q | --quiet] [<stash>]`, `version stash apply [--index] [-q | --quiet] [--label-ours=<label>] [--label-theirs=<label>] [--label-base=<label>] [<stash>]`, `version stash branch <branchname> [<stash>]`, `version stash [push] [-S | --staged] [-k | --[no-]keep-index] [-q | --quiet] [-u | --include-untracked] [-a | --all] [(-m | --message) <message>] [--pathspec-from-file=<file> [--pathspec-file-nul]] [--] [<pathspec>...]`, `version stash save [<options>] [<message>]`, `version stash store [(-m | --message) <message>] [-q | --quiet] <commit>`, `version stash create [<message>]`, `version stash clear` -- git's option surface, a port of builtin/stash.c.

Purpose: save and restore uncommitted work through `refs/stash`. A stash entry is git's three-commit shape (the working tree, its base and the index tree as parents, plus an untracked-files parent for `-u`), byte-identical to git's -- the entry's own message is stored without a trailing newline and the untracked parent is a root commit, as git writes them. `<stash>` is `stash@{N}`, a bare number, or any stash-like commit; `git stash` messages are git's throughout (`No stash entries found.`, `<rev> is not a valid reference`, `'<rev>' is not a stash-like commit`, `log for '<ref>' only has <n> entries`, `Too many revisions specified: ...`, `'<rev>' is not a stash reference`).

`push` (the default, assumed when the first argument is an option -- a stray word is git's `subcommand wasn't specified; 'push' can't be assumed due to unexpected token '<x>'`) saves the working tree and index and resets them, reporting `Saved working directory and index state <title>` or `No local changes to save`. `-m <msg>` titles the entry `On <branch>: <msg>` instead of `WIP on <branch>: <short> <subject>` (`(no branch)` when HEAD is detached); `-u`/`--include-untracked` adds the untracked files (every file under an untracked directory, removed with their directories afterwards), `-a`/`--all` adds the ignored ones too; `-k`/`--keep-index` leaves the staged content in place after the reset; `-S`/`--staged` stashes only the staged changes (taking them out of the index and the working tree); `-q` prints nothing. Pathspecs (after `--`, or from `--pathspec-from-file=<file>` with `--pathspec-file-nul`) limit what is reset, while the entry still records the whole index with the matched paths' working-tree content laid over it, as git's does; a pathspec matching nothing is git's `pathspec '<x>' did not match any file(s) known to git` plus `Did you forget to 'git add'?`. `--patch` is refused (there is no interactive hunk selector), and git's own refusals are kept: `Can't use --patch and --include-untracked or --all at the same time`, `Can't use --staged and --include-untracked or --all at the same time`, `options '--pathspec-from-file' and '--patch'/'--staged' cannot be used together`, `'--pathspec-from-file' and pathspec arguments cannot be used together`, `the option '--pathspec-file-nul' requires '--pathspec-from-file'`. The deprecated `save` form takes its message from the remaining words and no pathspecs.

`apply`/`pop` **merge** the stash onto the current tree, so they work on a dirty one: the stash's working tree is merged against its base with the index tree as the current side (git's `Updated upstream` / `Stashed changes` / `Stash base` labels, changed by `--label-ours`/`--label-theirs`/`--label-base`; `Version stash was based on` when the base is the current tree). The merge is narrated as git narrates it (`Auto-merging <path>`, `CONFLICT (content|add/add|binary|file/directory): Merge conflict in <path>`, `Already up to date.`) and followed by `git status`, both silenced by `-q`. A path whose working-tree content would be lost is git's `Your local changes to the following files would be overwritten by merge: ... Aborting` (exit 1). `--index` (also `stash.index`) restores the staged state as well, reporting `Index was not unstashed.` when the merge failed; without it the index keeps what it held, except for paths the stash adds anew. The untracked parent is unpacked back into the working tree (`could not restore untracked files from stash` when a path is in the way). `pop` drops the entry only on a clean apply, otherwise `The stash entry is kept in case you need it again.`; it refuses a stash that is not a `refs/stash` entry.

`list` takes git's `<log-options>` (it is `log --format=%gd: %gs -g --first-parent <options> refs/stash --`), so `--oneline`, `-n <n>`, `--format=<fmt>` and `--date=<mode>` all work. `show` takes git's `<diff-options>` (it is a `diff` between the stash's base and the stash), defaulting to `--stat` through `stash.showStat`/`stash.showPatch`; `-u`/`--include-untracked` diffs the untracked files too and `--only-untracked` just those, and paths after `--` limit it. `drop` prints `Dropped <rev> (<commit>)` and removes `refs/stash` once the last entry is gone; `branch <name> [<stash>]` creates the branch at the stash's base, checks it out (reporting the carried changes as `checkout -b` does), applies the stash with `--index` and drops it when that worked; `store` records a commit under `refs/stash` (default message `Created via "git stash store".`; storing the commit the ref already holds writes no new entry); `create` prints the stash commit without touching the ref or the working tree; `clear` removes every entry (`git stash clear with arguments is unimplemented`).
...
Syntax: `version worktree add [-f] [-q] [-d|--detach] [--[no-]checkout] [--lock [--reason <string>]] [--orphan] [(-b|-B) <new-branch>] [--track[=direct|inherit]|--no-track] [--[no-]guess-remote] <path> [<commit-ish>]`, `version worktree list [-v | --porcelain [-z]] [--expire <time>]`, `version worktree lock [--reason <string>] <worktree>`, `version worktree unlock <worktree>`, `version worktree move [-f] <worktree> <new-path>`, `version worktree remove [-f] <worktree>`, `version worktree prune [-n] [-v] [--expire <time>]`, `version worktree repair [<path>...]`, `version worktree current`.

Purpose: manage linked worktrees with shared objects/refs and isolated HEAD/index/sparse/replay state, matching `git worktree`. Each subcommand has its own option set; a usage error exits 129, a refusal 128, and a failing branch creation 255, as in git.

`add <path>` infers what to check out the way git does: the path's basename when it names a branch, otherwise a new branch of that name (from `<commit-ish>`, default `HEAD`) -- under `--guess-remote` (or `worktree.guessRemote`) a remote-tracking branch of that name starts it, and in a repository without commits git's `No possible source branch, inferring '--orphan'` applies instead. `-b`/`-B` name the new branch (`-B` resets an existing one, reporting `; was at <abbrev>`), `--detach` checks the commit out detached, and `--orphan` starts an unborn branch. `--track[=direct|inherit]`/`--no-track` choose the new branch's upstream (`branch.autoSetupMerge` otherwise), `--lock [--reason <string>]` leaves the new worktree locked (`added with --lock` when no reason is given), `--no-checkout` populates only the administrative files, and `-q` silences the `Preparing worktree (...)` and `HEAD is now at ...` lines. `-b`, `-B` and `--detach` are mutually exclusive, as are `--orphan` with `--detach`, `--track`, `--no-checkout` or a commit-ish; `--reason` requires `--lock`.

`list` prints git's layout -- `<path> <abbrev> [<branch>]`, `(detached HEAD)`, and the `locked`/`prunable` annotations, the main worktree first and the linked ones in path order. `-v` moves an annotation that carries a reason onto its own tab-indented line (`locked: <reason>`, `prunable: <reason>`); `--porcelain` prints the `worktree`/`HEAD`/`branch`/`detached`/`locked`/`prunable` record form (an unborn branch reports the null object id), and `-z` terminates each attribute with NUL. `--verbose` with `--porcelain`, and `-z` without it, are refused as git refuses them. `current` is this CLI's own spelling for the single record of the worktree the command runs in.

`lock`/`unlock` mark a worktree as one `remove`, `move` and `prune` must leave alone; the main worktree cannot be locked. `move` relocates a worktree and rewrites its `gitdir` pointer, `remove` deletes it with its administrative directory, and both need `-f -f` to override a lock (`-f` alone covers a dirty tree or an occupied destination). `prune` clears the administrative entries whose worktree directory is gone, reporting `Removing worktrees/<name>: <reason>` on standard error under `-n`/`-v`. `repair` re-points a worktree and its administrative entry at each other after the directory was moved by hand, reporting `repair: gitdir incorrect: <file>` for each pointer it corrected and diagnosing each path it cannot repair with `error:` and exit 1.

Common failures: branch already checked out elsewhere, target exists, dirty removal target, locked worktree, invalid common-dir or backlink metadata.
